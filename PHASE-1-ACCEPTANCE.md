...
| P1-03 | HTTP server test, fixed wire fixture, exact single `core.status`; default relative ST `DATA_ROOT` config path is normalized | NAS `/status` and frontend discovery/status presentation verified in actual SillyTavern |
...
| P1-17 | DOM status/refresh/preview/copy path, diagnostics whitelist and HTML escaping; single global gear menu reachable from AI/Worldbook/Oracle/Assets App | Phase 1 global settings entry verified in actual Web; Phase 2 Network mobile view pending |
...
| P1-21 | Installation checklist in README | **Partial real evidence:** NAS Core `/status` ready, 柏宝库 working, and direct Web fetch returns HTTP 200. Fixed frontend Client and timer receiver verified in actual Web; Phase 2 Network checks pending; no release acceptance claim |
